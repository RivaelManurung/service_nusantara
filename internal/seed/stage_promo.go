...
func (s *Seeder) seedBanners(ctx context.Context, _ Options) error {
	...
	for i, b := range seedBanners {
		banners = append(banners, model.Banner{
			...
			Photo:         assetURL(FolderBanners, b.Key),
			PhotoPublicID: assetPublicID(FolderBanners, b.Key),
	...
	events := []model.Event{
		{
			...
			Cover:         assetURL(FolderEvents, "pekan-kopi"),
			CoverPublicID: assetPublicID(FolderEvents, "pekan-kopi"),
			Status:        1,
			CreatedBy:     adminID,
		},
		{
			...
			Cover:         assetURL(FolderEvents, "hampers"),
			CoverPublicID: assetPublicID(FolderEvents, "hampers"),
			Status:        1,
			CreatedBy:     adminID,
		},
		{
			...
			Cover:         assetURL(FolderEvents, "batik"),
			CoverPublicID: assetPublicID(FolderEvents, "batik"),
			Status:        0,
			CreatedBy:     adminID,
		},
	}
